...
import OptimizedChatWindow from "./components/OptimizedChatWindow";
import OptimizedBigPrompt from "./components/OptimizedBigPrompt";
import OptimizedInputBox from "./components/OptimizedInputBox";
...
export default function OptimizedSpacingApp() {
  ...
  const loadSessions = useMemo(() => debounce(_loadSessionsCore, 300), [lang, email]);
  ...
  // Also reload whenever email changes
  useEffect(() => { if (email) loadSessions(email); }, [email]);
  ...
  const handleRename = async (s, name) => {
    try {
      ...
      if (selectedSessionId === s.session_id) {
        // update cache title only affects sidebar; no chat update needed
      }
      ...
      <ThemeProvider theme={theme}>
        ...
        >
          <Box className="app" sx={{ minHeight: '100vh', display: 'flex', flexDirection: 'column' }}>
            ...
            <Box className="layout" sx={{ display: 'flex', flex: 1, height: 'calc(100vh - 80px)' }}>
              {isSidebarOpen && (
